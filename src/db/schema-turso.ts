...
// Identical structure to schema-pg.ts but using SQLite column types.
// SQLite has no native boolean or bigint — uses integer with mode flags.
import {
  sqliteTable,
  text,
  integer,
...
export const schedules = sqliteTable(
  "schedules",
  {
    ...
    nextRun: integer("next_run", { mode: "number" }).notNull(),
...
export const loginLogs = sqliteTable(
  "login_logs",
  {
    ...
    credentialId: text("credential_id")
      .notNull()
      .references(() => credentials.id, { onDelete: "cascade" }),
    runTime: integer("run_time", { mode: "number" }).notNull(),
...
// Re-export PG types for TypeScript compatibility
export type {
  User,
  NewUser,
  Session,
